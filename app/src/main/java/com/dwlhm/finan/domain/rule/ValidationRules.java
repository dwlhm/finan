...
public final class ValidationRules {
    ...
    public static ValidationResult isValid(Transaction transaction) {
        ...
        if (transaction.getType().isRegular() && transaction.getCategoryId() <= 0L) {
            return ValidationResult.error("Category is required");
        }
        if (transaction.getType().isTransfer() && transaction.getTransferId() == null) {
            return ValidationResult.error("Transfer id is required");
        }
